...
class Yoochoose(object):
    ...
    def build_graphs(self, njobs = 1):
        ...
        self.num_clicks_dict = defaultdict(int)
        ...
        Parallel(n_jobs = njobs)(delayed(process_block)(self, njobs, i) for i in range(njobs))
        for sessionID in self.clicks.sessionIDs:
            timestamps = self.timestamp_dict[sessionID]
            timestamps.sort()
            duration = 0.0 if (len(timestamps) <= 1) else (timestamps[-1] - timestamps[0]).total_seconds()
            self.click_graph.node[sessionID].update({'numClicks' : self.num_clicks_dict[sessionID], 'duration' : duration})
        for itemID in self.clicks.itemIDs:
            self.click_graph.node[itemID]['numClicks'] = self.num_clicks_dict[itemID]
        self.buy_graph = nx.Graph()
        self.buy_graph.add_nodes_from(self.click_graph.nodes(data = True))
        for i in range(len(self.buys)):
            buy = self.buys.iloc[i]
            self.buy_graph.add_edge(buy['Session ID'], buy['Item ID'], timestamp = buy['Timestamp'], price = buy['Price'], quantity = buy['Quantity'])
...
def process_block(yoochoose, njobs, block_num):
    num_clicks = len(yoochoose.clicks)
    block_size = int(np.ceil(num_clicks / njobs))
    for i in range(block_num * block_size, max(num_clicks, (block_num + 1) * block_size)):
        click = yoochoose.clicks.iloc[i]
        sessionID = click['Session ID']
        itemID = click['Item ID']
        yoochoose.num_clicks_dict[sessionID] += 1
        yoochoose.num_clicks_dict[itemID] += 1
        yoochoose.timestamp_dict[sessionID].append(click['Timestamp'])
        yoochoose.click_graph.add_edge(sessionID, itemID, timestamp = click['Timestamp'], category = click['Category'])
